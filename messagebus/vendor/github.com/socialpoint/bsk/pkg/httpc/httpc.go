...
const requestMetricName = "httpc.request_duration"
...
// Decorate decorates a Client c with all the given Decorators, in reverse order.
...
// Instrument returns a Decorator that instrument requests with some metrics:
func Instrument(m metrics.Metrics, t ...metrics.Tag) Decorator {
	return func(c Client) Client {
		return ClientFunc(func(r *http.Request) (*http.Response, error) {
			timer := m.Timer(requestMetricName).WithTags(t...).WithTag("method", strings.ToLower(r.Method))
