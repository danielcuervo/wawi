...
var regexDateYYYYMMDDhm = regexp.MustCompile(`^(20[0-9]{2})-([0-9]{2})-([0-9]{2})(\s([0-9]{1,2}):([0-9]{2}))?`)
var regexDateDays = regexp.MustCompile(`^-([0-9]{1,2})\s?days?`)
var regexDateHours = regexp.MustCompile(`^-([0-9]{1,2})\s?hours?`)
var regexTimestamp = regexp.MustCompile(`^([0-9]{10})`)
...
func Parse(dateStr string, now time.Time) (time.Time, error) {
	...
	if match {
		return ParseFromDaysAgo(dateStr, now)
	}

	match = regexDateHours.MatchString(dateStr)
	if match {
		return ParseFromHoursAgo(dateStr, now)
	}
...
func ParseFromDaysAgo(dateStr string, now time.Time) (time.Time, error) {
	...
	return now.Add(-time.Duration(nbDay) * 24 * time.Hour), nil
}

// ParseFromHoursAgo returns Time from string with hours
//
// Example of valid inputs: "-1 hour" or "-10 hours"
func ParseFromHoursAgo(dateStr string, now time.Time) (time.Time, error) {
	...
	return now.Add(-time.Duration(nbHours) * time.Hour), nil
}
